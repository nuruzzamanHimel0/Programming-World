<?php 
	...
	public function saveWishListData($cmrId,$productId)
	{
		$cmrId = mysqli_real_escape_string($this->db->conn,$cmrId);
		$productId = mysqli_real_escape_string($this->db->conn,$productId);
		...
		$cquery = "SELECT * FROM tbl_wlist WHERE productId = '$productId' AND cmrId = '$cmrId' ";
			...
			if($cresult != FALSE)
			{
				$msg ="<span class='error' style='color:red;font-size:19px;font-weight:bold;'>Wish List Already Added </span>";
				 	return $msg;
			}
			else{
				$query = "SELECT * FROM tbl_product WHERE productId = '$productId' LIMIT 1 ";
			...
			if($result != FALSE)
			{
            	...
            	$compReslt = $this->db->insert($query);
            	 if($compReslt != FALSE)
				 {
				 	$msg ="<span class='success' style='font-size:19px;font-weight:bold;'>Add to Wish List </span>";
				 	return $msg;
				 }
				 else{
				 	$msg ="<span class='error' style='color:red;font-size:19px;font-weight:bold;'>Fail to Add Wissh List. </span>";
				 	return $msg;
				 }
			}
			}
	...
	public function dltWishlist($productId,$cmrId)
	{
		$dltquery = "DELETE FROM tbl_wlist WHERE productId = '$productId' AND cmrId = '$cmrId' ";
        $dltresult = $this->db->delete($dltquery);
	}
}
?>